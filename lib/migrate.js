...
Migration.prototype.run = function(options, callback) {
  ...
  Require.file({
    ...
    console.log("this is being called now")
    strFileUpdated = '---\n'
    ...
    strFileUpdated += "---\n"
    //strFileUpdated += "ifadjifesiofesiogeasjio"
    strFileUpdated += "---\n"
    strFileUpdated += deployer.content
    console.log(strFileUpdated);
    fs.truncate(self.file, 0, function() {
      fs.writeFile(self.file, strFileUpdated, function (err) {
        if (err) {
